...
private:
    ...
    struct ControlState {
        ...
        double manip_extend = 0.0;   // -1..1 удлинение
        double manip_height = 0.0;   // -1..1 высота
        bool   grip_closed = false;  // захват
    };
    ...
    // Кнопка сброса позы
    QPushButton *reset_pose_button_{nullptr};
    ...
    // Симуляция (для RobotViewWidget)
    ControlState control_;
    double sim_x_{0.0};
    double sim_y_{0.0};
    double sim_theta_{0.0};
    int    sim_dt_ms_{20};
};
