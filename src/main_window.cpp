      ...
      node_(std::move(node))
{
    ...
    // Инициализация геймпада (XInput через GamepadControl)
...
void MainWindow::setupTimers()
{
    ...
    connect(&sim_timer_, &QTimer::timeout,
            this, &MainWindow::onSimUpdate);
    sim_timer_.start();
}

void MainWindow::updateDiagnostics()
{
    // сюда можно добавить дополнительные обновления
}
...
// Обновление симуляции + публикация cmd_vel
...
void MainWindow::updateManipulatorModel(double dt)
{
    ...
    joint_positions_[0] += control_.manip_extend * speed_extend * dt;
    joint_positions_[1] += control_.manip_height * speed_height * dt;
    joint_positions_[2] += control_.manip_extend * speed_extend * dt * 0.5;
    joint_positions_[3] += control_.manip_height * speed_height * dt * 0.5;

    for (double &j : joint_positions_) {
        if (j > M_PI) j = M_PI;
        if (j < -M_PI) j = -M_PI;
    }
