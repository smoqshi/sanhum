...
void MotorDriver::applyPhaseForMotor(int fdA, int fdB,
                                     MotorDirection dir,
                                     int duty, int phase)
{
    ...
    // dutyNorm в (0,1]
    const double dutyNorm = std::clamp(duty / 100.0, 0.01, 1.0);

    // Период обратно пропорционален duty: чем меньше команда, тем длиннее период
    const double inv = 1.0 / dutyNorm;
    int period = static_cast<int>(minPeriod + (maxPeriod - minPeriod) * (inv - 1.0));
    ...
    const int localPhase = phase % period;
